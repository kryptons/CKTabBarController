...
@interface CKTabBar ()<UIScrollViewDelegate>
...
//    _numberBadgeFrame = CKTabItemBadgeFrameMake(2, 30, 16);
//    _dotBadgeFrame = CKTabItemBadgeFrameMake(5, 25, 10);
...
- (void)setItems:(NSArray *)items {
    ...
    // 初始化每一个item
    for (CKTabItem *item in self.items) {
...
- (void)setSelectedItemIndex:(NSInteger)selectedItemIndex {
    ...
    if (_selectedItemIndex >= 0) {
        CKTabItem *oldSelectedItem = self.items[_selectedItemIndex];
    ...
    CKTabItem *newSelectedItem = self.items[selectedItemIndex];
    ...
    if (self.delegate && [self.delegate respondsToSelector:@selector(CK_tabBar:didSelectedItemAtIndex:)]) {
        [self.delegate CK_tabBar:self didSelectedItemAtIndex:selectedItemIndex];
    }
...
- (void)updateFrameOfSelectedBgWithIndex:(NSInteger)index {
    ...
    CKTabItem *item = self.items[index];
...
- (void)tabItemClicked:(CKTabItem *)item {
    if (self.selectedItemIndex == item.index) {
        return;
    }
    BOOL will = YES;
    if (self.delegate && [self.delegate respondsToSelector:@selector(CK_tabBar:willSelectItemAtIndex:)]) {
        will = [self.delegate CK_tabBar:self willSelectItemAtIndex:item.index];
    }
    if (will) {
...
- (void)scrollViewDidScroll:(UIScrollView *)scrollView {
    ...
    CKTabItem *leftItem = self.items[leftIndex];
    CKTabItem *rightItem;
...
- (void)updateSeperators {
    if (self.itemSeparatorColor) {
        ...
        [self.items enumerateObjectsUsingBlock:^(CKTabItem * _Nonnull item, NSUInteger idx, BOOL * _Nonnull stop) {
            if (idx > 0) {
